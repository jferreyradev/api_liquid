module.exports.jsonViewMap = {    
    ...
    resumenSueldosComp: {
        fields: {
            ...
            Cat: "US_SUELDO.F_OBTIENE_CATEGORIA(LIQ.IDLIQ)",
            HabConAp: "ROUND(sum(CASE WHEN CONCEPTO.IDTIPOCONCEPTO = 1 THEN liqitem.IMPTICKET ELSE 0 END),2)",
            InasistRem: "ROUND(sum(case when CON.CODIGO = 160 and CON.SUBCOD = 0 then liqitem.impticket else 0 end),2)",
            HabConAp2: "ROUND( sum(case when CON.IDTIPOCONCEPTO = 1 THEN liqitem.impticket ELSE 0 END) - sum (case when CON.CODIGO = 160 and CON.SUBCOD = 0 then liqitem.impticket else 0 end) ,2)",
            HabSinAp: "ROUND(sum(CASE WHEN CONCEPTO.IDTIPOCONCEPTO = 2 THEN liqitem.IMPTICKET ELSE 0 END),2)",
